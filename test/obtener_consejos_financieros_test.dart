...
import 'package:finanzas_automaticas/domain/usecases/obtener_consejos_financieros.dart';
...
class _FakeConsejosFinancierosRepository
    implements ConsejosFinancierosRepository {
  ResumenParaConsejos? ultimoResumen;

  @override
  Future<List<String>> generarConsejos(ResumenParaConsejos resumen) async {
    ultimoResumen = resumen;
    return ['Consejo de prueba'];
  }
}

void main() {
  test(
    'el resumen para consejos es agregado y no contiene nombreDeuda/nombreAcreedor/nombres de cuenta',
    () async {
      ...
      final fakeConsejos = _FakeConsejosFinancierosRepository();
      final obtenerConsejos = ObtenerConsejosFinancieros(
        ...
        consejosFinancierosRepository: fakeConsejos,
      );

      final consejos = await obtenerConsejos();

      expect(consejos, ['Consejo de prueba']);
      final resumen = fakeConsejos.ultimoResumen;
      ...
      // disponibles en el resumen (los únicos que existen en el DTO) debe
      // contener los identificadores del acreedor, la deuda o la cuenta.

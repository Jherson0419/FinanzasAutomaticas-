...
- Botón "Iniciar sesión" (`FilledButton`) — deshabilitado hasta que el correo sea válido y la contraseña tenga ≥6 caracteres; invoca `AuthRepository.iniciarSesion`. **Fase 71:** justo después de un login por correo/contraseña exitoso, también invoca `RegistrarTokenDispositivo` (pide permiso de notificaciones push y, si se concede, guarda el token del dispositivo en Supabase) — envuelto en su propio `try/catch` silencioso, nunca bloquea el login ni se muestra como si hubiera fallado. No aplica al login con Google (mismo criterio que "Recuérdame": la sesión llega después, por el deep link).
...
- `DeudasActivasSection` (embebido, acordeón expandible por defecto; ahora `ConsumerStatefulWidget`, Fase 68, ya no un `StatefulWidget` a secas). **Fase 60:** el contenido expandido ya no es una sola lista con todas las deudas activas — se separa en 2 páginas deslizables por swipe horizontal según `estructuraPago` (cuotas fijas / pago libre), con un subtítulo que cambia según la página activa ("Cuotas fijas"/"Pago libre") y 2 puntitos indicadores tocables debajo del encabezado. El título "Deudas activas" y el total adeudado no cambian ni se duplican por página. Si una categoría no tiene ninguna deuda, esa página muestra "No tienes deudas de este tipo." — el mensaje general "No tienes deudas activas registradas." solo aparece si NO hay ninguna deuda activa de ningún tipo (ahí no se muestra el selector de páginas). **Fase 62:** la `Deuda` auto-generada de cada tarjeta de crédito (`estructuraPago = pagoLibre`) aparece como una fila más en la página "Pago libre", igual que cualquier otra. **Fase 68:** dentro de cada página, las filas ya no siguen el orden crudo del repositorio — `ObtenerResumenDashboard` las reordena (vencidas primero, luego por vencer en ≤3 días, luego el resto) y cada fila se resalta con fondo/borde `colorDanger`/`colorWarning` según corresponda (`urgenciaDeuda`, mismo umbral de 3 días que `AlertasTarjetasCreditoBanner`) — esto ya incluye a la deuda automática de tarjeta, cuya fecha real ahora se resuelve desde la `Cuenta` vinculada en vez de quedar siempre fuera de este cálculo. Si la deuda tiene `amigoUsuarioId` (Fase 64), una segunda línea dice "Debo a {nick}" (resuelto vía `amigosProvider`, Fase 63) — antes esta fila no mostraba ningún dato de acreedor.
- **Fase 68 — `TeDebenSection`** (embebido, entre "Deudas activas" y "Movimientos recientes"): deudas de OTROS usuarios vinculadas a mí como amigo (`amigo_usuario_id = auth.uid()`, política de RLS nueva — antes esas filas eran directamente invisibles para el amigo). Autocontenido, no se muestra nada si no hay ninguna (mismo criterio que `AlertasTarjetasCreditoBanner`). Cada fila es texto plano "{nick del deudor} te debe {monto}" — sin `InkWell`, sin swipe de pago, sin editar/eliminar: es de solo lectura, la deuda no es del usuario que la ve.
...
- Todos los datos de solo lectura vienen de `resumenDashboardProvider` (`dashboard_providers.dart`, que corre `ActualizarEstadoMora` y luego `ObtenerResumenDashboard`, tocando `CuentaRepository`, `TransaccionRepository`, `CategoriaRepository`, `DeudaRepository`, todos bifurcados Drift/Supabase según `datosEnLaNubeProvider`). **Fase 70:** después de calcular el resumen, el mismo provider llama a `GenerarNotificacionesVencimiento` (con `resumen.deudasActivas`, sin volver a consultar nada) para crear notificaciones de "cuota por vencer/vencida" como mucho una vez por día — invisible en esta pantalla, solo se nota en `NotificacionesScreen`/el badge de la campana si corresponde.
...
- Botón "Cerrar sesión" (`OutlinedButton.icon`) — con diálogo de confirmación que aclara que no se borran datos. **Fase 71:** antes de cerrar la sesión de Supabase, invoca `EliminarTokenDispositivoActual` (borra el token de push de este dispositivo, para no seguir recibiendo notificaciones de una cuenta de la que ya se salió) — envuelto en su propio `try/catch` silencioso, nunca bloquea cerrar sesión.
...
**Propósito:** listar las notificaciones dentro de la app — `solicitud_aceptada` (Fase 63), `pago_deuda_amigo` (Fase 64), `gasto_tarjeta`/`ingreso_recibido` (Fase 69, trigger sobre `transacciones`), `solicitud_recibida` (Fase 70, trigger sobre `solicitudes_amistad`), `cuota_por_vencer`/`cuota_vencida` (Fase 70, RPC `generar_notificaciones_vencimiento`, llamado desde `resumenDashboardProvider`), `deuda_pagada`/`deuda_amigo_pagada` (Fase 70, trigger sobre `deudas`) y desde la Fase 71 `deuda_vinculada` (trigger sobre `deudas`) — y marcarlas como leídas al tocarlas. Ninguna la inserta la app directamente: siempre un trigger o RPC de Postgres.
**Cómo se llega aquí:** ícono de campana del `DashboardScreen` (con badge de conteo de no leídas, ver entrada 12). **Fase 71** — también al tocar un push de Firebase con la app en segundo plano/cerrada, o si la app se abrió recién por uno (cold start): esta pantalla sigue siendo la única fuente de verdad de notificaciones, el push (`FinanzasAutomaticasApp`, `app.dart`) es solo un aviso adicional (`SnackBar` si la app estaba en primer plano) o una forma más de llegar aquí, nunca datos propios.

**Elementos visibles:**
- `ListView.separated` con una fila por notificación (`notificacionesProvider`), más reciente primero.
- **Fase 69, ampliado en las Fases 70 y 71** — cada fila tiene un ícono a la izquierda según `notificacion.tipo` (`_iconoParaTipoNotificacion`): personas (`solicitud_aceptada`), agregar persona (`solicitud_recibida`), pago (`pago_deuda_amigo`), tarjeta (`gasto_tarjeta`), ahorro (`ingreso_recibido`), reloj (`cuota_por_vencer`), alerta (`cuota_vencida`), check (`deuda_pagada`), celebración (`deuda_amigo_pagada`), enlace (`deuda_vinculada`); un ícono genérico de campana para cualquier tipo no listado, nunca falla ante un tipo desconocido.
...
**Sistema de diseño:** consistente — usa `colorSuccess` del sistema de tokens de la Fase 19/31 para el punto de no leída; los íconos de tipo usan `colorScheme.onSurfaceVariant`, sin colores nuevos fuera del sistema.
